...
<nav class="navbar navbar-inverse navbar-fixed-top">
		...
		<?php 
			...
			$query = "SELECT username FROM company WHERE company_id =(SELECT company_id FROM interview )";
			$result = mysqli_query($con, $query);

			
			
			while($question_Row = mysqli_fetch_array($result)){
				echo '<p><form action="interviews.php" method="post"><button name="submit" type="submit" value="'.$question_Row[0].'"">'.$question_Row[0].''." ".'</button></form></p>';
			}
