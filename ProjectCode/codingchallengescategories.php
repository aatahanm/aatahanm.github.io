...
<nav class="navbar navbar-inverse navbar-fixed-top">
		...
		<div class="container">
		<div class="jumbotron">
    		<h2>Categories for Coding Challenges</h2>
  		</div>
		...
		<?php 
			...
			$query = "SELECT category_name, COUNT(*) AS count FROM category_coding_challenges GROUP BY category_name";
			$result = mysqli_query($con, $query);

			
			
			while($question_Row = mysqli_fetch_array($result)){
				echo '<p><form action="codingchallenges.php" method="post"><button name="submit" type="submit" value="'.$question_Row[0].'"">'.$question_Row[0].''." ".''.$question_Row[1].'</button></form></p>';
			}
